<!doctype html> <html lang="en"> <head> <meta charset="UTF-8"> <title>Test</title>
    ...
    <script>
        ...
        angular.module('battleService', [])
            ...
            .factory('Battle', function($http, $rootScope) {
                ...
                return {
                    ...
                    action : function(id, target, spell) {
                        return $http({
                            ...
                            function (response)
                            {
                                $rootScope.$broadcast('battleUpdated', response.data);
                            }
                        );
                    },
        ...
        app.controller('battleController', function($scope, $http, $log, Battle) {
            // object to hold all the data for the new comment form
            $scope.battleData = function () { return Battle.battleData};
            ...
            $scope.testAction = function() {
                console.log('inside controller '+$scope.test);
                $scope.test = Battle.testAction($scope.test);
                console.log('inside controller '+$scope.test);
            }
            ...
            $scope.$on('battleUpdated', function(event, data) {
                console.log('battleUpdated event call recieved');
...
<body  ng-app="spellcasterApp" ng-controller="battleController"> <div class="col-md-8 col-md-offset-2">
    ...
    <div>
        ...
        <div ng-repeat="fighter in battle.fighters track by fighter.id">
            <input type="radio" name="actionData.target" ng-model="actionData.target" value="{{fighter.id}}" ng-if="fighter.user.id != 1 && fighter.hp > 0"/>
            {{fighter.identifier}}
            <span ng-if="fighter.hp > 0">({{fighter.hp}} HP)</span>
            <span ng-if="fighter.hp <= 0">(DEAD)</span>
        </div>
